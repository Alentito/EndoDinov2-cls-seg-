...
def frames_to_video(frames, output_path, fps=5, placeholder_shape=(320,320,3)):
    """
    Convert list of RGB frames to a video file.
    If frames list is empty, create a 1-frame black video.
    """
    if frames is None or len(frames) == 0:
        # create black placeholder frame
        black_frame = np.zeros(placeholder_shape, dtype=np.uint8)
        frames = [black_frame]
...
def process_image(image_path):
    ...
    report = {
        ...
        "segmentation_available": seg_overlay is not None
    }

    heatmap_video = frames_to_video(
    [overlay],
    "temp_image_heatmap.mp4",
    fps=1
)

    seg_video = None
    if seg_overlay is not None:
        seg_video = frames_to_video(
            [seg_overlay],
            "temp_image_seg.mp4",
            fps=1
        )

    return heatmap_video, seg_video, report
...
def process_video(video_path):
    ...
    heatmap_video_file = frames_to_video(heatmaps, "temp_heatmap.mp4", fps=FPS_SAMPLE)
    seg_video_file = frames_to_video(
    [f for f in seg_overlays if f is not None], 
    "temp_segmentation.mp4", 
    fps=FPS_SAMPLE,
    placeholder_shape=(frames[0].shape if len(frames)>0 else (UNET_IMAGE_SIZE,UNET_IMAGE_SIZE,3))
)
...
# ---------------- GRADIO ----------------
with gr.Blocks(title="Endoscopy Classifier + Segmentation") as iface:
    gr.Markdown("# Endoscopy Image & Video Classifier + Polyp Segmentation")
    with gr.Row():
        with gr.Column(scale=1):
            input_file = gr.File(label="Upload Image or Video")
        with gr.Column(scale=1):
            out_heatmap = gr.Video(label="Classification + GradCAM Overlay")
            out_seg = gr.Video(label="Polyp Segmentation Overlay (if polyp)")
            out_report = gr.JSON(label="Prediction / Segmentation Report")

    input_file.change(
        lambda f: process_image_or_video(f.name),
        inputs=[input_file],
        outputs=[out_heatmap, out_seg, out_report]
    )
